...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-lg shadow p-8">
        <h1 class="text-3xl font-bold mb-8">Edit Pemain</h1>
        ...
        <form method="POST" action="{{ route('admin.players.update', $player) }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" class="w-full border border-gray-300 rounded-lg px-4 py-2" required value="{{ $player->name }}">
                @error('name')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="jersey_number" name="jersey_number" class="w-full border border-gray-300 rounded-lg px-4 py-2" required value="{{ $player->jersey_number }}">
                @error('jersey_number')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="position" name="position" class="w-full border border-gray-300 rounded-lg px-4 py-2" required>
                    <option value="Point Guard" {{ $player->position === 'Point Guard' ? 'selected' : '' }}>Point Guard</option>
                    <option value="Shooting Guard" {{ $player->position === 'Shooting Guard' ? 'selected' : '' }}>Shooting Guard</option>
                    <option value="Small Forward" {{ $player->position === 'Small Forward' ? 'selected' : '' }}>Small Forward</option>
                    <option value="Power Forward" {{ $player->position === 'Power Forward' ? 'selected' : '' }}>Power Forward</option>
            ...
            <div>
                ...
                <input type="number" id="height" name="height" class="w-full border border-gray-300 rounded-lg px-4 py-2" value="{{ $player->height }}">
                @error('height')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="weight" name="weight" class="w-full border border-gray-300 rounded-lg px-4 py-2" value="{{ $player->weight }}">
                @error('weight')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" id="birth_date" name="birth_date" class="w-full border border-gray-300 rounded-lg px-4 py-2" value="{{ $player->birth_date?->format('Y-m-d') }}">
                @error('birth_date')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="biography" name="biography" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-2">{{ $player->biography }}</textarea>
                @error('biography')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
