            ...
            <div data-aos="fade-up" class="hidden lg:flex justify-center">
                <div class="relative">
                    ...
                    <div class="relative bg-white/5 backdrop-blur-lg rounded-3xl p-10 border-2 border-[#FFD700]/40 hover:border-[#FFD700] transition-all">
                        <img src="{{ asset('images/logos/LOGO GARUDA HUSTLER.png') }}"
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            ...
            <div class="group" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <div class="relative bg-gradient-to-br from-[#1a1f35] to-[#0d1b2a] rounded-2xl shadow-md shadow-[#FFD700]/10 hover:shadow-2xl hover:shadow-[#FFD700]/30 transition-all transform hover:scale-105 overflow-hidden h-full border border-[#FFD700]/20">
                    <!-- Background Gradient -->
                    <div class="absolute inset-0 bg-gradient-to-br {{ $stat['color'] }} opacity-0 group-hover:opacity-10 transition-opacity"></div>

                    <!-- Content -->
                    <div class="relative p-6 md:p-8 flex flex-col items-center justify-center text-center">
                        <div class="text-4xl md:text-5xl mb-3">{{ $stat['icon'] }}</div>
                        <div class="text-3xl md:text-4xl font-black bg-gradient-to-r {{ $stat['color'] }} bg-clip-text text-transparent mb-2">
                            {{ $stat['number'] }}
                        </div>
                        <div class="text-gray-300 font-semibold text-sm md:text-base">{{ $stat['label'] }}</div>
                    </div>

                    <!-- Bottom Border Accent -->
                    <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r {{ $stat['color'] }} transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></div>
                </div>
            </div>
            @endforeach
        </div>
...
<section class="py-20 bg-gradient-to-b from-[#0d1b2a] to-[#1a1f35]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-gradient-to-br from-[#1a1f35] to-[#0d1b2a] rounded-2xl shadow-lg shadow-[#FFD700]/10 overflow-hidden hover:shadow-2xl hover:shadow-[#FFD700]/30 transition-all duration-300 transform hover:scale-105 border border-[#FFD700]/20">
                    ...
                    <div class="relative h-40 bg-gradient-to-br from-[#FFD700]/30 to-[#FF8C00]/30 flex items-center justify-center overflow-hidden border-b border-[#FFD700]/20">
                        ...
                        <div class="relative text-center">
                            <div class="text-6xl md:text-7xl font-black text-[#FFD700] drop-shadow-lg">{{ $player->jersey_number }}</div>
                            <div class="text-[#FF8C00] font-bold text-sm md:text-base mt-2 drop-shadow">{{ $player->position }}</div>
                        </div>
                    </div>

                    <!-- Content -->
                    <div class="p-5">
                        <h3 class="font-bold text-lg mb-3 line-clamp-2 text-[#FFD700]">{{ $player->name }}</h3>
                        ...
                        <div class="space-y-2 text-sm text-gray-300">
                            ...
                            <div class="flex justify-between items-center py-2 border-b border-[#FFD700]/20">
                                ...
                                <span class="font-bold text-[#FFD700]">{{ $player->height }} cm</span>
                            </div>
                            @endif
                            @if($player->weight)
                            <div class="flex justify-between items-center pt-2">
                                <span class="text-gray-400">Berat</span>
                                <span class="font-bold text-[#FF8C00]">{{ $player->weight }} kg</span>
                            </div>
                            @endif
                        </div>
                        @endif
        ...
        <div class="text-center mt-16">
            <a href="{{ route('team') }}" class="inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-[#FFD700] to-[#FF8C00] text-[#0d1b2a] font-bold rounded-xl hover:from-[#FF8C00] hover:to-[#FFD700] transition-all transform hover:scale-105 shadow-lg">
                Lihat Semua Pemain
...
<section class="py-20 bg-gradient-to-b from-[#1a1f35] to-[#0d1b2a]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-gradient-to-r from-[#1a1f35] to-[#0d1b2a] rounded-lg shadow shadow-[#FFD700]/10 p-6 hover:shadow-lg hover:shadow-[#FFD700]/30 transition border border-[#FFD700]/20" data-aos="fade-up">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    <div>
                        <div class="text-sm text-gray-400">{{ $match->match_date->format('d M Y') }}</div>
                        <div class="font-bold text-lg text-[#FFD700]">{{ $match->match_date->format('H:i') }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-400">GARUDA HUSTLER</div>
                        <div class="font-bold text-[#FFD700]">{{ $match->team_score ?? '-' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-sm text-gray-400">{{ ucfirst($match->type) }}</div>
                        <div class="font-bold text-[#FF8C00]">{{ $match->opponent }}</div>
                    </div>
                    <div>
                        <div class="font-bold text-[#FFD700]">{{ $match->opponent_score ?? '-' }}</div>
                        <div class="text-sm text-gray-400">{{ $match->location }}</div>
                    </div>
                </div>
            </div>
            @empty
        ...
        <div class="text-center mt-12">
            <a href="{{ route('schedule') }}" class="bg-gradient-to-r from-[#FFD700] to-[#FF8C00] text-[#0d1b2a] px-8 py-3 rounded-lg font-bold hover:from-[#FF8C00] hover:to-[#FFD700] transition inline-block shadow-md">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="rounded-lg overflow-hidden shadow-lg shadow-[#FFD700]/10 hover:shadow-xl hover:shadow-[#FFD700]/30 transition transform hover:scale-105 cursor-pointer border border-[#FFD700]/20" data-aos="zoom-in">
                <img src="{{ asset($gallery->image) }}" alt="{{ $gallery->title }}" class="w-full h-64 object-cover">
                <div class="p-4 bg-gradient-to-br from-[#1a1f35] to-[#0d1b2a]">
                    <h3 class="font-bold text-lg mb-2 text-[#FFD700]">{{ $gallery->title }}</h3>
                    <p class="text-gray-400 text-sm line-clamp-2">{{ $gallery->description }}</p>
                </div>
            </div>
            @empty
        ...
        <div class="text-center mt-12">
            <a href="{{ route('gallery') }}" class="bg-gradient-to-r from-[#FFD700] to-[#FF8C00] text-[#0d1b2a] px-8 py-3 rounded-lg font-bold hover:from-[#FF8C00] hover:to-[#FFD700] transition inline-block shadow-md">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-gradient-to-br from-[#1a1f35] to-[#0d1b2a] rounded-lg shadow-lg shadow-[#FFD700]/10 overflow-hidden hover:shadow-xl hover:shadow-[#FFD700]/30 transition border border-[#FFD700]/20" data-aos="fade-up">
                ...
                <img src="{{ asset($article->featured_image) }}" alt="{{ $article->title }}" class="w-full h-48 object-cover">
                @endif
                <div class="p-6">
                    <div class="text-sm text-gray-400 mb-2">
                        {{ $article->created_at->format('d M Y') }}
                    </div>
                    <h3 class="font-bold text-xl mb-3 line-clamp-2 text-[#FFD700]">{{ $article->title }}</h3>
                    <p class="text-gray-400 mb-4 line-clamp-3">{{ strip_tags($article->content) }}</p>
                    <a href="{{ route('news.detail', $article->slug) }}" class="text-[#FF8C00] font-bold hover:text-[#FFD700] transition">
                        Baca Selengkapnya →
                    </a>
                </div>
            </div>
        ...
        <div class="text-center mt-12">
            <a href="{{ route('news') }}" class="bg-gradient-to-r from-[#FFD700] to-[#FF8C00] text-[#0d1b2a] px-8 py-3 rounded-lg font-bold hover:from-[#FF8C00] hover:to-[#FFD700] transition inline-block shadow-md">
                Lihat Semua Berita
            </a>
        </div>
    </div>
